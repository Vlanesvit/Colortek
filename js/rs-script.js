...
wrapMenuText('.rs-header__menu .menu__list a')
wrapMenuText('.rs-footer .rs-footer__spollers_body ul li a')
...
function marquee() {
	...
	marquees.forEach(marquee => {
		...
		const speed = 0.5; // Скорость прокрутки бегущей строки
		let scrollAmount = 0; // Переменная для отслеживания текущего смещения

		// Вычисляем общую ширину или высоту всех элементов для создания бесшовного эффекта
		const totalSize = marquee.dataset.direction === "up" || marquee.dataset.direction === "bottom"
			? items.reduce((acc, item) => acc + item.offsetHeight, 0)
			: items.reduce((acc, item) => acc + item.offsetWidth, 0);

		const resetOffset = totalSize + 215; // Добавляем смещение для плавного сброса

		// Дублируем элементы один раз для создания бесшовного эффекта, добавляя их в конец списка
		if (marquee.dataset.direction === "up" || marquee.dataset.direction === "bottom") {
			list.style.height = `${totalSize * 2}px`;
		} else {
			list.style.width = `${totalSize * 2}px`;
		}
		...
		function scrollMarquee() {
			...
			if (marquee.dataset.direction === "left") {
				...
				scrollAmount += speed;
			} else if (marquee.dataset.direction === "up") {
				scrollAmount -= speed;
			} else if (marquee.dataset.direction === "bottom") {
				scrollAmount += speed;
			}
